...
bool JOIN::optimize(bool finalize_access_paths) {
  ...
  */
  ...
  // Extract complete semantic WHERE conjuncts before join-order search.
  semantic_plan_refiner =
      std::make_unique<vipersql::SemanticPlanRefiner>(thd);
  // Outer joins and semijoins require MySQL's native condition placement.
  if (!query_block->outer_join && query_block->sj_nests.empty()) {
    where_cond = semantic_plan_refiner->ExtractSemanticConjuncts(where_cond);
  }
  has_semantic_operators = !semantic_plan_refiner->empty();
  ...
  */
  ...
  // Attach semantic predicates at the stages selected during plan search.
  if (has_semantic_operators &&
      semantic_plan_refiner->ReplayWinningPlacement(this)) {
    return true;
  }
...
static const char *can_switch_from_ref_to_range(THD *thd, JOIN_TAB *tab,
  ...
  if ((tab->range_scan() &&                                            // 1)
        ...
        !dep_map)       // 4)
    {
      if (recheck_range)  // 6)
      {
      ...
      } else if (length == get_max_used_key_length(tab->range_scan()) &&
        ...
        /*
          ...
          interface
        */
        auto qr = tab->range_scan()->index_range_scan();
        if ((qr.mrr_flags & HA_MRR_USE_DEFAULT_IMPL) == 0 &&
            (qr.mrr_flags & HA_MRR_CONVERT_REF_TO_RANGE) != 0) {
          return "uses_mrr";
        }
      } else {
        return length < get_max_used_key_length(tab->range_scan())
                   ? "uses_more_keyparts"
                   : nullptr;  // 5)
      }
    }
