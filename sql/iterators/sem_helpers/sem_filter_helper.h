#ifndef SQL_ITERATORS_EXTERNAL_HELPERS_SEMHELPERS_FILTER_HELPER_H_
#define SQL_ITERATORS_EXTERNAL_HELPERS_SEMHELPERS_FILTER_HELPER_H_
...
/**
 * SemFilterHelper
 * A configurable semantic filter helper that calls different backends
 * by switching the model name in the request JSON.
 */
class SemFilterHelper : public ExternalHelperInterface {
public:
  explicit SemFilterHelper(std::string model_name);
  ~SemFilterHelper() override;
...
private:
  ...
  std::vector<uint8_t> m_results;        // parsed boolean results
...
#endif  // SQL_ITERATORS_EXTERNAL_HELPERS_SEMHELPERS_FILTER_HELPER_H_
