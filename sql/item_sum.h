...
class Item_sum : public Item_func {
 ...
 public:
  // how many args does this SUM/COUNT have?  COUNT(*) will be zero.
  unsigned arg_num_count() const { 
    // MySQL keeps NULL-terminated args[];
    // so count until you hit a nullptr
    unsigned cnt = 0;
    for ( ; args[cnt]; ++cnt ) { }
    return cnt;
  }
  // get the i'th argument (for COUNT(*) i==0 is invalid)
  Item *arg(unsigned i) const { return args[i]; }
